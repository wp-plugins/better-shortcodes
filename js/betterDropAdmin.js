	...
	if(obj){		
		for (var i = 0; i < l; i++) {
			...
			for (var key in shortcodes) {
				var tmp_stop  = shortcodes[key].lastIndexOf('_');
				var checked   = parseInt(shortcodes[key].substring(tmp_stop + 1,shortcodes[key].length));
				
				if(l==1){
					addValues(key,decodeURI(shortcodes[key].substring(0,tmp_stop)),checked,1);
				}
				else if(l>1) {
					if(j==4){
						addValues(key,decodeURI(shortcodes[key].substring(0,tmp_stop)),checked,1);
						...
						addValues(key,decodeURI(shortcodes[key].substring(0,tmp_stop)),checked,j);
						j++;
					}
				}
	 ...
	 */	
 	function combineFields(){
		...
		for(var j = 0; j < savedNames.length; j++){
			tmp_j = '{"'+savedNames[j]+'":"'+savedCodes[j]+'_'+savedClose[j]+'"}';
